...
// ==================== Alerts Helpers ====================
export async function listAlerts(filters?: { severity?: string; isRead?: boolean; status?: "active" | "resolved" | "dismissed"; startTime?: number; endTime?: number; limit?: number; offset?: number }) {
  const db = await getDb();
  if (!db) return { data: [], total: 0 };
  const conditions = [];
